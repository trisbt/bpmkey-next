...
export default async function SiteMap() {
    ...
    for (const href of featHrefs) {
        ...
        const playlistResults = playlistTrackData.map(track => {
            if (!track || !track.name || !track.artists || track.artists.length === 0 || !track.artists[0].name) {
                return null;  
            }
    
            const slugifiedUrl = `https://www.bpmkey.com/${slugify(track.name, { lower: true, strict: true })}/${slugify(track.artists[0].name, { lower: true, strict: true })}/${track.id}`;
            return {
                url: slugifiedUrl,
                lastModified: new Date(),
            };
        ...
        allResults.push(...playlistResults);
    }
